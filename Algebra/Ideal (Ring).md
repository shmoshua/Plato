...
- **Related Definition**: For a subset $S\subseteq R$, $$\braket{ S  } :=\bigcap_{\mathfrak{a}\subseteq R\text{ ideal},S\subseteq \mathfrak{a}}^{}\mathfrak{a}$$is the ***ideal generated by $S$*** in $R$.
- **Related Definition**: For ideals $\mathfrak{a},\mathfrak{b}\subseteq R$: $$\begin{align}\mathfrak{a+b}&=\{ x+y: x\in \mathfrak{a},y\in \mathfrak{b} \}\\\mathfrak{a\cdot b}&=\left\langle\{ xy: x\in \mathfrak{a},y\in\mathfrak{b} \}\right\rangle=\left\{  \sum_{i=1}^{n}x_{i}y_{i}:n\in \mathbb{N}, x_{i}\in \mathfrak{a},y_{i}\in \mathfrak{b}  \right\}\end{align}$$
