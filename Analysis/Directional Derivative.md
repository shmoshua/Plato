...
> 1. For a convex $f:\mathbb{R}^n\to \mathbb{R}$, the ***directional directive*** of $f$ at $x$ is given by:$$f'(x;v):=\lim_{ t \to 0^+ } \frac{f(x+tv)-f(x)}{t}$$
...
> 1. $f'(x;\cdot)$ is convex. 
> 2. $f'(x;\alpha v)=\alpha f'(x;v)$

> [!proof]-
> We have that: 
> 1. $$\begin{aligned}f'(x;\lambda u+(1-\lambda)v)&=\lim_{ t \to 0^+ }\frac{ f(x+t\lambda u+t(1-\lambda)v)-f(x)}{t}\\&\leq\lim_{ t \to 0^+ }\frac{ \lambda f(x+tu)+(1-\lambda)f(x+tv)-f(x)}{t}\\&=\lambda f'(x;u)+(1-\lambda)f'(x;v)\end{aligned}$$
> 2. $f'(x;\alpha v)=\lim_{ t \to 0^+ }\alpha\frac{f(x+t\alpha v)-f(x)}{t\alpha}=\alpha f'(x;v)$.
---
