...
> For all $T\in A$, so $Tv=0$ if and only if $\widehat{T}=0$ $E_{v,v}$-almost everywhere. Hence, $$\mathcal{L}_{v}:Av\to L^2(\widehat{A},E_{v,v}),\quad Tv\mapsto \widehat{T}$$is a well-defined linear isometry. This extends to $\mathcal{L}_{v}:\overline{Av}\to L^2(\widehat{A},E_{v,v})$ which is a linear isometry and injective since $\mathcal{L}_{v}(Av)=C(\widehat{A})$ and $C(\widehat{A})$ is dense in $L^2(\widehat{A},E_{v,v})$.
> 
> Let $S,T\in A$. Then, $\mathcal{L}_{v}(TSv)=\widehat{T}\widehat{S}=\widehat{T}\mathcal{L}_{v}(Sv)$. Therefore, for any $w\in Av$ and $\mathcal{L}_{v}(Tw)=\widehat{T}\mathcal{L}_{v}(w)$ and by continuity, this extends to all $w\in \overline{Av}$. 
> 
> Let $J\subseteq \mathbb{N}$ from the previous lemma. Then, $\mathcal{H}=\widehat{\bigoplus}_{n\in J}\overline{Av_{n}}$. Modulo rescaling, we may assume that $\sum_{n\in J}^{}\left\| v_{n} \right\|^{2}<+\infty$. Define $f\in C_{0}(\widehat{A}\times J)$. Then, $$\int_{\widehat{A}\times J} f  \, d\mu:=\sum_{n\in J}^{}\int_{\widehat{A}}^{} f(x,n) \, dE_{v_{n},v_{n}}  $$which defines the measure $\mu$ by Riesz representation. Further, $\mu$ is finite as: $$\mu(\widehat{A}\times J)=\sum_{n\in J}^{}E_{v_{n},v_{n}}(\widehat{A})=\sum_{n\in J}^{}\left\| v_{n} \right\| ^{2}<+\infty$$Therefore, we can define a Hilbert space isomorphism $\Lambda:\mathcal{H}\to L^{2}(\widehat{A}\times J,\mu)$ where: $$\Lambda(w)(\chi,n)=\sum_{n\in J}^{}\mathcal{L}_{v_{n}}(w_{n})(\chi)$$where $w=\sum_{n\in J}^{}w_{n}$ with $w_{n}\in \overline{Av_{n}}$. Now, we have: $$\Lambda(Tw)(\chi,n)=\sum_{n\in J}^{}\mathcal{L}_{v_{n}}(Tw_{n})(\chi)=\sum_{n\in J}^{}\widehat{T}(\chi)\mathcal{L}_{v_{n}}(w_{n})(\chi)=\widehat{T}(\chi)\Lambda(w)(\chi,n)$$
