...
> [!proof]-
> Let $(\rho_{t})_{t=0}^T$ be a decreasing sequence s.t. $\rho_{T}=\rho$ and $\rho_{0}=1$. Then, $$\widehat{Z}(\rho,\lambda)=\widehat{Z}(\rho_{0},\lambda)\prod_{t=1}^{T}\frac{\widehat{Z}(\rho_{t},\lambda)}{\widehat{Z}(\rho_{t-1},\lambda)}=(1+\lambda)^{\left| E \right| }\prod_{t=1}^{T}\mathbb{E}_{F\sim \widehat{\mu}_{\rho_{t-1},\lambda}}\left[ \left( \frac{\rho_{t}}{\rho_{t-1}} \right)^{\left| \textsf{odd}(F) \right| } \right]$$Hence, we can use the FPAS to estimate each $\mathbb{E}_{F\sim \widehat{\mu}_{\rho_{t-1},\lambda}}\left[ \left( \frac{\rho_{t}}{\rho_{t-1}} \right)^{\left| \textsf{odd}(F) \right| } \right]$. Since $\rho_{t}$ is decreasing, $(\rho_{t} / \rho_{t-1})^{\left| \textsf{odd}(F) \right|}$ is a $1$-bounded function. Now, suppose $\rho_{t}\leq \rho_{t-1}\leq \rho_{t}+\frac{1}{n}$. We show that: $$\mathbb{E}_{F\sim \widehat{\mu}_{\rho_{t-1},\lambda}}\left[ \left( \frac{\rho_{t}}{\rho_{t-1}} \right)^{\left| \textsf{odd}(F) \right| } \right]\ge \Omega(1)$$Let $C\in (0,1)$ be some constant. Then, 
>1. If $\rho_{t-1}\geq C$, we have that: $$\frac{\rho_{t}}{\rho_{t-1}}=1-\frac{\rho_{t-1}-\rho_{t}}{\rho_{t-1}}\geq 1-\frac{1}{Cn}$$Hence, $(\rho_{t} / \rho_{t-1})^{\left| \textsf{odd}(F) \right|} \geq\left( 1-\frac{1}{Cn} \right)^n=\Omega(1)$
>2. If $\rho_{t-1}\leq C$, we have that: $$\begin{aligned}\mathbb{E}_{F\sim \widehat{\mu}_{\rho_{t-1},\lambda}}\left[ \left( \frac{\rho_{t}}{\rho_{t-1}} \right)^{\left| \textsf{odd}(F) \right| } \right]&=\frac{\widehat{Z}(\rho_{t},\lambda)}{\widehat{Z}(\rho_{t-1},\lambda)}\\&=\left( \frac{\cosh(h_{t-1})}{\cosh(h_{t})} \right)^{\left| V \right| }\frac{Z(\beta,h_{t})}{Z(\beta,h_{t-1})}\\&\ge\left( \frac{\cosh(h_{t-1})}{\cosh(h_{t})} \right)^{\left| V \right| }\cdot \min_{x}\exp((h_{t}-h_{t-1})\braket{ x , 1 } )\\&=\left( \frac{\cosh(h_{t-1})}{\cosh(h_{t})} \right)^{n }\cdot \exp((h_{t}-h_{t-1})n )\\&=\exp(n(\phi(h_{t-1})-\phi(h_{t})))\end{aligned}$$where $\phi(x)=\log \cosh(x)-x$. Since $\rho_{t-1}\leq C$, $\left| h_{t-1}-h_{t} \right|\leq O(1/n)$ and since $\phi$ is $O(1)$-Lipschitz, we get the $\Omega(1)$ lower bound. 
>
>Therefore, we can take $T \approx (1-\rho)n$ and $\rho_{t}=\rho_{t-1}-\frac{1}{n}$ for all $t$.

---
> [!lemma] Theorem 3 
> There is a FPAS for sampling from $\widehat{\mu}_{\rho,\lambda}$ for every $0\leq \rho,\lambda\leq 1$. More specifically, Glauber dynamics has mixing time: $$\text{T}_{mix}(\varepsilon)\leq O\left( \frac{m^{2}}{\rho^6}(m+\log(1/\varepsilon)) \right)$$