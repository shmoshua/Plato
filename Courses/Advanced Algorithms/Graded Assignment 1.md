...
---
**Lemma**: $\frac{x+z}{\min\{ x,y \}}\geq 1+\frac{z}{y}$. 
 
Let $\varepsilon>0$ and choose $X\in \mathbb{N}$ s.t. $\frac{1}{X}<\varepsilon/\sqrt{ 2 }$. Let $Y:= \sqrt{ 2 }\cdot X$. We show that $\mathcal{A}$ is not $(1+\sqrt{ 2 }-\varepsilon)$-competitive. 
...
---
If $\left| E_{R} \right|,\left| E_{B} \right|\leq\left\lceil \frac{1}{3}\left| E \right|\right\rceil-1$ Then, $$|E_{R}|+|E_{B}|\leq 2\left\lceil \frac{1}{3}|E|\right\rceil-2 $$Therefore, $|E_{V}|\ge |E|-2\left\lceil \frac{1}{3}|E|\right\rceil+2$. Hence,

$$\begin{aligned}\mathbb{E}[X]&=\sum_{e\in E_R}\mathbb{P}(v\text{ or }w\text{ is blue})+\sum_{e\in E_B}\mathbb{P}(v\text{ or }w\text{ is red})+\sum_{e\in E_V}\mathbb{P}(v,w\text{ has the same color})\\&\leq \frac{3}{4}(|E_R|+|E_B|)+\frac{1}{2}|E_V|\\&\le \frac{3}{4}(|E|-|E_V|)+\frac{1}{2}|E_V|\\&=\frac{3}{4}|E|-\frac{1}{4}|E_V|\\&\le\frac{3}{4}|E|-\frac{1}{4}|E|+\frac{1}{2}\left\lceil \frac{1}{3}|E|\right\rceil -\frac{1}{2}\\&=\frac{1}{2}|E|+\frac{1}{2}\left\lceil \frac{1}{3}|E|\right\rceil-\frac{1}{2}\end{aligned}$$Therefore, by Markov,$$\mathbb{P}\left( \left| E \right| -X< \frac{\text{OPT}}{3} \right)=\mathbb{P}\left( X>|E|-\frac{\text{OPT}}{3} \right)\leq \frac{\mathbb{E}[X]}{\left| E \right| -\frac{\text{OPT}}{3}} \leq \frac{\mathbb{E}[X]}{\frac{2}{3}\left| E \right|} \leq 1-\frac{3}{8|E|}$$Now, 

---



If $|E_R|\ge  m/3$, by coloring every vertex red, we have that: $$\mathcal{A}(I)=|E_R|\ge m/3\ge \text{OPT}/3$$Same holds if $|E_B|\ge  m/3$ with the blue coloring. \\

Now assume that $\left| E_{R} \right|,\left| E_{B} \right|< m/3$. Then, $$|E_{R}|+|E_{B}|\leq 2\left\lceil \frac{m}{3}\right\rceil-2 $$and $|E_{R}|+|E_{B}|\le  \frac{2}{3}m-2$. Therefore, $|E_{V}|\ge m-2 \left\lceil \frac{m}{3}\right\rceil+2$. Now we will analyze the probability for a single instance of the randomized coloring. Let $X$ denote the number of edges for which we did not receive a point. Then, 

$$\begin{aligned}\mathbb{E}[X]&=\sum_{e\in E_R}\mathbb{P}(v\text{ or }w\text{ is blue})+\sum_{e\in E_B}\mathbb{P}(v\text{ or }w\text{ is red})+\sum_{e\in E_V}\mathbb{P}(v,w\text{ has the same color})\\&\leq \frac{3}{4}(|E_R|+|E_B|)+\frac{1}{2}|E'_V|\\&\le \frac{3}{4}(m-|E_V|)+\frac{1}{2}|E_V|\\&\le \frac{3}{4}m-\frac{1}{4}|E_V|\\&\le\frac{3}{4}m-\frac{1}{12}m-\frac{1}{4}\\&=\frac{2}{3}m-\frac{1}{4}\end{aligned}$$Therefore, by Markov,$$\mathbb{P}\left( m -X< \frac{\text{OPT}}{3} \right)=\mathbb{P}\left( X>m-\frac{\text{OPT}}{3} \right)\leq \frac{\mathbb{E}[X]}{m -\frac{\text{OPT}}{3}} \leq \frac{\mathbb{E}[X]}{\frac{2}{3}m} \leq 1-\frac{3}{8m}$$Now, by running this algorithm $L$ times, we have that: 
