...
3) We have that: $$\begin{align}\mathbb{E}[\sigma(j)Z_{h(j)}]&=\mathbb{E}\left[ \sum_{i\in[j]}^{}\sigma(j)\sigma(i) f_{i}\cdot \mathbb{1}_{h(i)=h(j)}\right] \\&=\sum_{i\in[j]}\mathbb{E}[\sigma(j)\sigma(i)\mathbb{1}_{h(i)=h(j)}]f_{i}\\&=\sum_{i\in[j]}\mathbb{E}[\sigma(j)\sigma(i)|h(i)=h(j)]\mathbb{P}(h(i)=h(j))f_{i}\end{align}$$