...
We will show that if an algorithm $A$ uses at most $m:=\min\{ \frac{\log n}{400 \varepsilon },\log n-1 \}$ bits then there exists an input $N$ for which the algorithm fails.

We will consider the following two cases:
1. $\varepsilon(n)\leq \frac{1}{4}$ 
   We will now construct the following streams. Let $\mathcal{B}$ denote the family of all subsets of $[n]$ of size $k:=\left\lfloor \frac{1}{2 \varepsilon}\right\rfloor\geq 1$. For each $B\in \mathcal{B}$, we can order them s.t. $B=\{ a_{1},\dots,a_{k} \}$ where $a_{1}<a_{2}<\dots<a_{k}$. Then, the stream associated to $B$ is given by: $$S_{B}:=(a_{1},\dots,a_{k})$$ 
...
$\frac{n}{k}\geq \frac{n\varepsilon}{\varepsilon+1}>\frac{n^{0.01}}{1+\varepsilon}$

We have that $\frac{1}{\varepsilon}\leq k< \frac{1}{\varepsilon}+1$

$$\frac{n}{k}> \frac{\varepsilon n}{\varepsilon+1} >\frac{n^{0.01}}{n^{-0.99}(1+n^{0.99})}$$
