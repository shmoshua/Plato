...
5. As $h$ is both convex and $\beta$-gradient Lipschitz, by Theorem 3.3.7 we have our Gradient Descent algorithm s.t. for $x_{0}\in \mathbb{R}^n$ and the unique minimizer $x^{*}$, the sequence $(x_{i})_{i}$ given by $$x_{i+1}:=x_{i}-\frac{1}{\beta}\nabla h(x_{i})$$satisfies: $$f(x_{i})-f(x^{*})\leq \frac{2\beta \left\| x_{0}-x^{*} \right\| ^2_{2}}{i+1},\quad \forall i\in \mathbb{Z}_{\geq 0}$$where for each $i$ we only need to compute the gradient of $h$ once. Therefore, by 4, we can find a point $\widehat{x}\in \mathbb{R}^n$ with $\left\| \widehat{x}-x^{*} \right\|_{2}\leq \delta$ with computing gradients at most $\frac{16\beta}{\mu}\log(\|x_{0}-x^{*}\|_{2} / \delta)$ points. 
   
   Now, notice that for any point $x'\in \mathbb{R}^n$, $$h(x')-h(x^{*})\leq \underbrace{ \nabla h(x^{*})^\top }_{ =0 }(x'-x^{*})+\frac{\beta}{2}\left\| x'-x^{*} \right\| ^2_{2}=\frac{\beta}{2}\left\| x'-x^{*} \right\| ^2_{2}$$Hence, it suffices to find a point $x'\in \mathbb{R}^n$ s.t. $\left\| x'-x^{*} \right\|_{2}\leq \frac{2\varepsilon}{\beta \left\| x_{0}-x^{*} \right\|_{2}}$. Hence, $$\frac{16\beta}{\mu}\log\left( \frac{\beta\left\| x_{0}-x^{*} \right\| _{2}^{2}}{2\varepsilon} \right)$$
