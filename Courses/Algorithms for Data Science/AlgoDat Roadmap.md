...
> 3. $M_{3}:=\sum_{j}^{}\alpha_{j}(\mu^0_{j})^{\otimes  3}+M_{1} \otimes  \left( \sum_{r=1}^{d}e_{r}\otimes  e_{r} \right)+ \sum_{r\in[d]} (e_{r}\otimes  M_{1} \otimes  e_{r})+\left( \sum_{r=1}^{d}e_{r}\otimes  e_{r} \right) \otimes  M_{1}$

> [!proof]-
> We have that: 
> 4. Notice that: $$\begin{aligned}\mathbb{E}\left[ \frac{1}{n}\sum_{i}^{}y_{i} \right]&=\frac{1}{n}\sum_{i} \mu^0_{z^0(i)}+\underbrace{ \mathbb{E}[w_{i}] }_{ =0 }=\frac{1}{n}\sum_{i}^{}\mu^0_{z^0(i)}=\frac{1}{n}\sum_{i}\sum_{j\in[k]}^{}\mu^0_{j}\mathbb{1}_{z^0(i)=j}=\sum_{j}^{}\alpha_{j}\mu^0_{j}\end{aligned}$$
> 5. Notice that: $$\begin{aligned}\mathbb{E}\left[ \frac{1}{n}\sum_{i}^{}(y_{i}\otimes  y_{i}) \right] &=\frac{1}{n}\sum_{i}^{}(\mu^0_{z^0(i)})^{\otimes  2}+\mathbb{E}[w_{i}\otimes  w_{i}]+\underbrace{ \mathbb{E}[\mu^0_{z^0(i)}\otimes  w_{i}]+\mathbb{E}[ w_{i}\otimes  \mu^0_{z^0(i)}] }_{ =0 }\\&=\sum_{j}\alpha_{j}(\mu^0_{j})^{\otimes  2}+\sum_{r=1}^{d}e_{r}\otimes e_{r}\end{aligned}$$
> 6. Notice that: $$\begin{aligned}(a+b)^{\otimes  3}&=(a+b)\otimes (a^{\otimes  2}+b^{\otimes  2}+a\otimes  b+b\otimes  a)\\&=a^{\otimes  3}+b^{\otimes  3}+a\otimes  b^{\otimes  2}+a^{\otimes  2}\otimes  b+a\otimes  b\otimes  a+b\otimes  a^{\otimes  2}+b\otimes  a\otimes  b+b^{\otimes  2}\otimes  a\end{aligned}$$Therefore, if $w_{i}$ appears once in the tensor, its expectation is $0$. Further, $$\mathbb{E}[w_{i}^{\otimes  3}]_{p,q,r}=\mathbb{E}[w_{i,p}w_{i,q}w_{i,r}]=\begin{cases}0&p\neq q\neq r\\0&\text{two are equal}\\0&\text{all are the same}\end{cases}$$Further, we have that: $$\mathbb{E}[w_{i}\otimes  \mu^0_{j}\otimes  w_{i}]_{p,q,r}=\mu^0_{j,q}\mathbb{E}[w_{i,p}w_{i,r}]=\mu^0_{j,q}\mathbb{1}_{p= r}=\sum_{\ell}^{}(e_{\ell}\otimes  \mu^0_{j}\otimes  e_{\ell})_{p,q,r}$$Therefore, $$\begin{aligned}M_{3}&=\frac{1}{n}\sum_{i}\left[ (\mu^0_{z^0(i)})^{\otimes  3}+\underbrace{ \mathbb{E}[w_{i}^{\otimes  3}] }_{ =0 }+ \mu^0_{z^0(i)}\otimes  \mathbb{E}[w_{i}^{\otimes  2}]+ \mathbb{E}[w_{i}\otimes  \mu^0_{z^0(i)}\otimes w_{i}]+\mathbb{E}[w_{i}^{\otimes  2}]\otimes   \mu^0_{z^0(i)}\right] \\&=\sum_{j}^{}\alpha_{j}(\mu^0_{j})^{\otimes  3}+M_{1} \otimes  \left( \sum_{r=1}^{d}e_{r}\otimes  e_{r} \right)+ \sum_{r\in[d]} (e_{r}\otimes  M_{1} \otimes  e_{r})+\left( \sum_{r=1}^{d}e_{r}\otimes  e_{r} \right) \otimes  M_{1}\end{aligned}$$where $$\begin{aligned}\frac{1}{n}\sum_{i}^{}\mathbb{E}[w_{i}\otimes  \mu^0_{z^0(i)}\otimes  w_{i}]&=\sum_{j}^{}\alpha_{j}\mathbb{E}[w_{i}\otimes  \mu^0_{j}\otimes  w_{i}]\\&=\sum_{j}\alpha_{j}\sum_{r}^{}e_{r}\otimes  \mu^0_{j}\otimes  e_{r}=\sum_{r\in [d]}^{}e_{r}\otimes  M_{1} \otimes  e_{r}\end{aligned}$$

- **Remark**: We can compute $\sum_{j}^{}\alpha_{j}(\mu^0_{j})^{\otimes  3}$ if we have $M_{1},M_{2},M_{3}$.
...
> Suppose $(X^0,Y,z^0)$ satisfy $\mathcal{A}(Y-X^0)\cup \mathcal{B}(X^0,z^0)$. Then, $$\mathcal{A}(Y-X)\cup \mathcal{B}(X,z)\mathrlap{\ \ ^{^{X,z}}}\mathrlap{\ \ _{_{O(t)}}}\vdash\ \ \text{err}(X)\leq 4t\cdot  k^{4 / t}$$
...
>    Let $E:=\frac{1}{n}\sum_{i\in[n]}w_{i}^{\otimes  t} -\mathbb{E}[w^{\otimes  t}]$. Then, we have that: $$\begin{aligned} \frac{1}{n}\sum_{i\in[n]}^{}\braket{ w_{i} , v } ^t&=\frac{1}{n}\sum_{i\in[n]}^{}\braket{ v^{\otimes  t / 2} , w_{i}^{\otimes  t}v^{\otimes  t / 2} } \\&=\braket{ v^{\otimes  t / 2} , E v^{\otimes  t / 2} }+\braket{ v^{\otimes  t / 2} , \mathbb{E}[w^{\otimes  t}] v^{\otimes  t / 2} } \\&=\braket{ v^{\otimes  t / 2} , E v^{\otimes  t / 2} }+\mathbb{E}[\braket{ w , v } ^t]\\&\leq\braket{ v^{\otimes  t / 2} , E v^{\otimes  t / 2} }+(t-1)!!\|v\|^{t}\\&\leq \frac{1+\left\| E \right\| ^{2}_{F}}{2}\|v^{\otimes  t /2}\|^{2}_{F}+(t-1)!!\|v\|^t\\&\leq (0.505+(t-1)!!)\|v\|^t\\&\leq t^{t / 2}\|v\|^t\end{aligned}$$where as $v^\top w\sim \mathcal{N}(0,\|v\|^{2})$, we have $\mathbb{E}[\braket{ w , v } ^t]\leq (t-1)!!\|v\|^{t}$.
>    
>    Hence, $$\begin{aligned}\mathcal{A}(W)\mathrlap{\ \ ^{^{X,v}}}\mathrlap{\ \ _{_{}}}\vdash\ \frac{1}{n}\sum_{i\in[n]}\braket{ u_{i} , v } ^t&=\frac{1}{n}\sum_{i\in[n]}^{}(\braket{ w^0_{i} , v } -\braket{ w_{i} , v } )^t\\&\leq 2^{t-1}\frac{1}{n}\sum_{i\in[n]}^{}\braket{ w^0_{i} , v } ^t+\braket{ w_{i} , v } ^t\\&\leq 2^t t^{t /2}\|v\|^t\end{aligned}$$
> 2. **Claim 2: $\mathcal{A},\mathcal{B}\mathrlap{\ \ ^{^{X,z}}}\mathrlap{\ \ _{_{}}}\vdash\ \frac{1}{n}\sum_{i=1}^{n}\|u_{i}\|^{t}\leq k^{2}(4t)^{t/2}$**
>    Let $z^0_{ib}:=\mathbb{1}_{X^0_{i}=\mu^0_{b}}$ and $z_{iab}:=z_{ia}z^0_{ib}$. Then, as $\mathcal{B}\mathrlap{\ \ ^{^{X,z}}}\mathrlap{\ \ _{_{}}}\vdash\ z_{iab},z_{jab} (U_{i}-U_{j}) =0$, we have: $$\begin{aligned}\mathcal{B}\mathrlap{\ \ ^{^{X,z}}}\mathrlap{\ \ _{_{}}}\vdash\ z_{iab}z_{jab}\braket{ U_{i} , U_{j} } &=\sqrt{z_{iab}z_{jab} \braket{ U_{i} , U_{j} } }\sqrt{z_{iab}z_{jab} \braket{ U_{i} , U_{j} } } \\&=z_{iab}z_{jab}\|U_{i}\|\|U_{j}\|\end{aligned}$$Hence,$$\begin{aligned}\mathcal{B}\mathrlap{\ \ ^{^{X,z}}}\mathrlap{\ \ _{_{}}}\vdash\ z_{i,a,b}z_{j,a,b}\braket{ u_{i} , u_{j} } ^t&=(z_{i,a,b}z_{j,a,b}\braket{ u_{i} , u_{j} } )^t\\&=z_{iab}z_{jab}\left\| U_{i} \right\| ^t\left\| U_{j} \right\| ^t\end{aligned}$$Summing over $i$ and applying Claim 1, $$\begin{aligned}\mathcal{A},\mathcal{B}\mathrlap{\ \ ^{^{X,z}}}\mathrlap{\ \ _{_{}}}\vdash\ z_{jab}\left\| U_{j} \right\| ^t \frac{1}{n}\sum_{i=1}^{n}z_{iab}\left\| U_{i} \right\| ^t&=z_{jab} \frac{1}{n}\sum_{i\in[n]}^{}z_{iab}\braket{ u_{i} , u_{j} } ^t\\&\leq z_{jab} \frac{1}{n}\sum_{i\in[n]}^{}\braket{ u_{i} , u_{j} } ^t\\&\leq z_{jab}(4t)^{t/2}\left\| u_{j} \right\| ^t\end{aligned}$$Averaging this over all $j$, $$\mathcal{A,B}\mathrlap{\ \ ^{^{X,z}}}\mathrlap{\ \ _{_{}}}\vdash\ \left( \frac{1}{n}\sum_{i=1}^{n}z_{iab} \left\| u_{i} \right\| ^t\right)^{2}\leq (4t)^{t/2}\frac{1}{n}\sum_{i\in[n]}^{}z_{iab}\left\| u_{i} \right\| ^t $$Hence, by Proposition 1.8, $$\mathcal{A,B}\mathrlap{\ \ ^{^{X,z}}}\mathrlap{\ \ _{_{}}}\vdash\ \frac{1}{n}\sum_{i\in[n]}^{}z_{iab}\left\| u_{i} \right\| ^t\leq (4t)^{t/2}$$Summing over all choices $a,b$ we have that: $$\mathcal{A,B}\mathrlap{\ \ ^{^{X,z}}}\mathrlap{\ \ _{_{}}}\vdash\ \frac{1}{n}\sum_{i\in[n]}^{}\left\| u_{i} \right\| ^t\leq k^{2}(4t)^{t/2}$$
>  
>  Finally, we have that for any $C>0$, $$\mathcal{A},\mathcal{B}\mathrlap{\ \ ^{^{X,z}}}\mathrlap{\ \ _{_{}}}\vdash\ \frac{1}{n}\left\| U \right\| ^{2}_{F}=\frac{1}{n}\sum_{i=1}^{n}\left\| u_{i} \right\| ^{2}\leq \frac{2}{t}\frac{\frac{1}{n}\sum_{i=1}^{n}\left\| u_{i} \right\| ^t}{C^{t-2}}+\frac{t-2}{t}C^2\leq \frac{2}{t}\frac{k^{2}(4t)^{t/2}}{C^{t-2}}+\frac{t-2}{t}C^2$$By choosing $C^t=k^{2}(4t)^{t/2}$, we have that: $$\mathcal{A,B}\mathrlap{\ \ ^{^{X,z}}}\mathrlap{\ \ _{_{}}}\vdash\ \frac{1}{n}\left\| U \right\| ^{2}_{F}\leq C^{2}=4t\cdot k^{4/t}$$
...
> [!proof]-
> We have that: $$\left\| \overline{H} -\overline{G}\right\|_{\text{cut}}\leq \sum_{i\neq j}^{} \left| \overline{H}_{ij}-\overline{G}_{ij} \right| \lesssim \text{\#edges where }G\text{ and }H\text{ differ}\leq \eta n^{2}$$Hence, $$\text{ex-err}(\widehat{x})\leq \frac{\eta n}{\varepsilon d}$$
