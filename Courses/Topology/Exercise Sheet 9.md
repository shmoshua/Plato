...
3. Let $x+H_{k}\in X_{n,k}$ for $x\in \mathbb{R}^n$. Then, $p|_{C}:C\to p(C)$ is a continuous bijection and as $C$ is compact and $p(C)\subseteq X_{n,k}$ is Hausdorff, $p|_{C}$ is a homeomorphism. This proves the statement.
4. Let $\varphi:\mathbb{R} / \mathbb{Z} \to S^1$ be the homeomorphism seen in lecture. Then, $$\begin{array}{cccc} {:}&{X_{n,k}}&\to&{(\mathbb{R} / \mathbb{Z})^k\times \mathbb{R}^{n-k}}&\to&{(S^1)^k\times \mathbb{R}^{n-k}}\\&{x+H_{k}} &\mapsto & {(x_{1} + \mathbb{Z},\dots,x_{k} + \mathbb{Z},x_{k+1},\dots,x_{n})}&\mapsto & {(\varphi(x_{1} + \mathbb{Z}),\dots,\varphi(x_{k} + \mathbb{Z}),x_{k+1},\dots,x_{n})} \end{array}{}$$is a homeomorphism as a series of homeomorphisms.
---