...
> 3. (1=>3): Let $M$ be orientable with orientation $o$. We will show that this is continuous. Let $p\in M$ and let $(p,o_{p})\in \text{O}M$. Further, let $\text{O}U_{o_{U}}$ be an open neighborhood of $(p,o_{p})$. Then, let $V$ be the open neighborhood of $p$ with a continuous local frame $E_{1},\dots,E_{m}$.  By taking a connected open $W\subseteq V\cap U$, we have that: $o_{U}|_{W}$ and $o|_{V}$ are two orientations that coincide on $p$. By Proposition 4, $o_{U}|W=o|_{V}$ and $o(V)\subseteq \text{O}U_{o_{U}}$. This shows that $o$ is continuous. 
> 4. (3=>4): Consider the map: $$\Psi:\text{O}M\to M\times \{ \pm 1 \},\quad (p,o_{p})\mapsto\left(p,\begin{cases}1&o_{p}=o(p)\\-1&o_{p}\neq o(p)\end{cases}\right)$$Then, $\Psi=(\pi,\psi)$ where $\psi ^{-1}(\{ 1 \})=o(M)$ which is open. However it is also closed and $\Psi$ is continuous. We then have that: $\Psi ^{-1}(p,b)=\begin{cases}o(p)&b=1\\-o(p)&b=-1\end{cases}$ which is continuous as $o$ and consequently $-o$ is continuous. 
---
