...
>    
>    
>    Recall $\rho_{A}(x):=\mathbb{C} \backslash \text{Sp}_{A}(x)$ is an open subset of $\mathbb{C}$. Then, for $\ell\in A^{*}$, we define: $$f:\rho_{A}(x)\to \mathbb{C},\quad \lambda\mapsto \ell((\lambda e-x)^{-1})$$ We first try to show that $f$ is holomorphic. For $\lambda,\mu\in\rho_{A}(x)$ with $\lambda\neq \mu$ we have:$$\frac{f(\lambda)-f(\mu)}{\lambda-\mu}=\frac{\ell((\lambda e-x)^{-1}-(\mu e-x)^{-1})}{\lambda-\mu }$$where $$((\lambda e-x)^{-1}-(\mu e-x)^{-1})(\lambda e-x)(\mu e-x)=\mu e-x-\lambda e+x=(\mu-\lambda)e$$ and $(\lambda e-x)^{-1}-(\mu e-x)^{-1}=-(\lambda-\mu)((\lambda e-x)(\mu e-x))^{-1}$. It follows that: $$\frac{f(\lambda)-f(\mu)}{\lambda-\mu}=-\ell((\mu e-x)^{-1}(\lambda e-x)^{-1})$$and by continuity of inversion, $\lim_{ \mu \to \lambda }\frac{f(\lambda)-f(\mu)}{\lambda-\mu}=-\ell((\lambda e-x)^{-2})$. Hence, $f:\rho_{A}(x)\to \mathbb{C}$ is holomorphic.
>    
>    We are left to show that $f$ is bounded on $\rho_{A}(x)$. Let $\left| \lambda \right|\leq 2\|x\|$. Then, $$\left| f(\lambda) \right| \leq \left\| \ell \right\| \left\| \lambda e-x \right\| \leq \left\| \ell \right\|\|x\| (2\|e\|+1) $$Conversely, if $\left| \lambda \right|>2\|x\|$, then $r_{A}\left( \frac{x}{\lambda} \right)=\frac{r_{A}(x)}{\left| \lambda \right|}< \frac{1}{2}$ and: $$\left| f(\lambda) \right| =\left| \ell((\lambda e-x)^{-1}) \right| =\left| \ell\left( \sum_{n=0}^{\infty}\frac{x^n}{\lambda^{n+1}} \right) \right| \leq \frac{1}{\left| \lambda \right| }\sum_{n=0}^{\infty}\left\| \ell \right\| \left\| \frac{x}{\lambda} \right\| ^n<\frac{2\left\| \ell \right\|}{\left| \lambda \right| }<\frac{\left\| \ell \right\| }{\|x\|}$$
