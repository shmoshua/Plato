...
>    
>    ---
>    To show the converse, we claim that for any $q\in \Delta(\mathcal{X})$, $I(q,W)-\varepsilon$ is achievable for all $\varepsilon>0$. Let $\mathcal{F}_{n}:=\{ f:\mathcal{M}_{n}\to \mathcal{X}^n \}$ be the set of all possible encoders. Let $f_{n}\sim \mathcal{F}_{n}$ with some underlying distribution. Then, define:$$\overline{\lambda}_{m}:=\mathbb{E}[\lambda_{m}(f_{n})]$$Then, $$\mathbb{E}[P^n_{e}(f_{n})]=\mathbb{E}\left[ \frac{1}{2^{nR}}\sum_{m}^{}\lambda_{m}(f_{n}) \right]=\frac{1}{2^{nR}}\sum_{m}^{}\mathbb{E}[\lambda_{m}(f_{n})]=\frac{1}{2^{nR}}\sum_{m}^{}\overline{\lambda}_{m}$$Now, let $Y$ be the outcome of the channel of $f_{n}(1)$. Define $\varepsilon_{i}$ as the event that $(f_{n}(i),Y)\in \mathcal{A}^n_{\varepsilon}(q \times W)$. Then, consider the following decoder: $$\phi_{n}(y):=\begin{cases}x&\text{if }\{ \xi\in \mathcal{X}^n:(\xi,\eta)\in \mathcal{A}^n_{\varepsilon}(q\times W) \}=\{ x \}\\{\bot}&\text{otherwise}\end{cases}$$Then, the event of an error is given by: $\varepsilon_{1}^c\cup \bigcup_{i=2}^{2^{mR}} \varepsilon_{i}$. 
>    
