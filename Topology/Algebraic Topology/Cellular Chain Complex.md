...
- **Related definition**: For a CW-complex $K$ and a subcomplex $A\subseteq K$, the ***relative cellular chain complex*** is defined as follows: ^891341
...
> 2. We have that for $\sigma\in I_{n}$: $$\begin{align}\Phi \circ \beta_{n}\circ \Psi(\sigma)&=\Phi \circ j_{n-1}\circ \partial_{n} \circ (f_{\sigma})_{*}[I^n]=\Phi \circ j_{n-1}\circ (f_{\partial\sigma})_{*}[\partial I^n]\\&=\sum_{\tau\in I_{n-1}}^{}\phi_{n-1}((p_{\tau})_{*}\circ j_{n-1}\circ (f_{\partial\sigma})_{*}[\partial I^n])\tau\\&=\sum_{\tau\in I_{n-1}}^{}\phi_{n-1}((p_{\tau}\circ f_{\partial \sigma})_{*}[\partial I^n])\tau\end{align}$$We conclude by noting that $\phi_{n-1}((p_{\tau}\circ f_{\partial\sigma})_{*}[\partial I^n])=\deg(p_{\tau} \circ f_{\partial\sigma})$ by definition. 
