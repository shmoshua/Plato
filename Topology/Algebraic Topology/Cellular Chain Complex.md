...
> 2. $d_{n}:=\Phi_{n-1}\circ\beta_{n}\circ\Psi_{n}$ where $\Psi,\Phi$ are the isomorphisms given in Proposition 1 and $\beta_{n}:H_{n}(K^{(n)},K^{(n-1)})\to H_{n-1}(K^{(n-1)},K^{(n-2)})$ is given by [[CW-Complex|Proposition 8]].
---
##### Properties
> [!lemma] Proposition 1
> For a CW-complex $K$, 
...
> 2. $\mathcal{C}(K)$ is a chain complex. 
> 3. $H_{n}(\mathcal{C}(K))\cong H_{n}(K)$ for all $n\in \mathbb{Z}$.
...
> 2. Holds from the fact that $\beta_{n}\circ\beta_{n+1}=0$ from [[CW-Complex|Proposition 8]] and $\Psi \circ\Phi=\text{id}$.
> 3. We have that: $$H_{n}(\mathcal{C}(K))=\text{ker}(d_{n}) / \text{im}(d_{n+1})\cong \text{ker}(\beta_{n}) / \text{im}(\beta_{n+1})\cong H_{n}(K^{(n+1)})\cong H_{n}(K)$$