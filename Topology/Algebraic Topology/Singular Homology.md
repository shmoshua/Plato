...
>       Let $f:X\to X'$ and $g:Y\to Y'$. Then, let $\sigma\in S_{n}(X\times Y)$. We have that: $$(f_{c}\otimes g_{c}) \circ ((\pi_{X}\circ \sigma)_{c}\otimes  (\pi_{Y}\circ \sigma)_{c})(\Theta d_{n})=(-1)^{\left| g_{c} \right| \left| (\pi_{X}\circ \sigma)_{c} \right| }(f\circ  \pi_{X} \circ  \sigma)_{c}\otimes  (g \circ  \pi_{Y}\circ  \sigma)_{c}(\Theta d_{n})$$and: $$\Theta((f\times g)_{c}(\sigma))=((\pi_{X}\circ  ))$$