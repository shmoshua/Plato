...
> 1. The ***blow-up*** of a real algebraic set $W$ with center $V\subseteq W$ is defined as: $$B_{V}(W):=\overline{\{ (x,(x_{1}:\dots:x_{r}))\in W \times \mathbb{R}\mathbb{P}^{r-1} : x\in W \backslash V\}}$$
...
> 2. $\pi ^{-1}(v)\cong\mathbb{R}\mathbb{P}^{r-1}$ for all $v\in V$.

> [!proof]+
> We have that:
> 1. Surjectivity is clear by definition. If $\pi((x,(x_{1}:\dots:x_{r})))=\pi((y,(y_{1}:\dots:y_{r})))$, then $x=y$. Hence, it is injective.
> 2. Let $v=(0,\dots,0,v_{r+1},\dots,v_{d})\in V$ and let $p=(a_{1}:\dots:a_{r})\in \mathbb{R}\mathbb{P}^{r-1}$. Then, define: $x^{(n)}:=\left( \frac{a_{1}}{n},\dots, \frac{a_{r}}{n},v_{r+1},\dots,v_{d} \right)$. Then, $$(x^{(n)},p)\xrightarrow{n\to\infty}(v,p)$$Hence, $(v,p)\in B_{V}(\mathbb{R}^d)$ and we have the statement. 