...
---
> [!lemma] Lemma 4
> Let $f:X\to Y$ be a [[Continuous Function|continuous function]] and a convergent filter $\mathcal{F}\to x$. Then,
> 1.  $f_{*}(\mathcal{F})$ converges to $f(x)$.

> [!proof]-
> Let $U$ be an open neighborhood of $f(x)$. Then, $f^{-1}(U)$ is an open neighborhood of $x$ in $X$ and there exists $V\in \mathcal{F}$ s.t. $V\subseteq f^{-1}(U)$. Therefore, $U\in f_{*}(\mathcal{F})$.
---
> [!lemma] Lemma 5
> Let $\mathcal{F}$ be a filter on $\prod_{i\in I}^{}X_{i}$. Then, TFAE:
> 1. $\mathcal{F}$ converges to $x=(x_{i})_{i}$ 
> 2. $(p_{i})_{*}(\mathcal{F})$ converges to $x_{i}$ for all $i\in I$.

^0214a6

> [!proof]-
> As $p_{i}$ is continuous, by Lemma 4, $(p_{i})_{*}(\mathcal{F})$ converges to $p_{i}(x)=x_{i}$.
> 
> Conversely, assume that $(p_{i})_{*}(\mathcal{F})$ converges to $x_{i}$ for all $i\in I$. Let $U$ be a neighborhood of $x$. Then, there exists a finite $J\subseteq I$ and open $U_{j}\subseteq X_{j}$ neighborhood of $x_{j}$ for $j\in J$ s.t. $$\bigcap_{j\in J}^{}p_{j}^{-1}(U_{j})\subseteq U$$As $(p_{j})_{*}(\mathcal{F})\to x_{i}$, we have that $p ^{-1}_{j}(U_{j})\in \mathcal{F}$ and thereby $\bigcap_{j\in J}^{}p_{j}^{-1}(U_{j})\in \mathcal{F}$.
---
##### Examples
